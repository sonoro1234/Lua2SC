...
local function prof_top(count1, count2, samples, indent)
  ...
  for i=1,n do
    ...
    if not prof_raw then
      table.insert(out,format("%s%2d%%  %s\n", indent, pct, k))
    elseif prof_raw == "r" then
      table.insert(out,format("%s%5d  %s\n", indent, v, k))
    else
      table.insert(out,format("%s %d\n", k, v))
    end
    if count2 then
      local r = count2[k]
      if r then
...
local function prof_annotate(count1, samples)
  ...
  for _, file in ipairs(files) do
    ...
    if not fp then
      table.insert(out,format("====== ERROR: %s: %s\n", file, err))
      break
    end
    table.insert(out,format("\n====== %s ======\n", file))
    local fl = files[file]
    ...
    if ann ~= 0 then
      for i=1,ann do
	if fl[i] then show = true; table.insert(out,"@@ 1 @@\n"); break end
      end
    end
    for line in fp:lines() do
      if line:byte() == 27 then
	table.insert(out,"[Cannot annotate bytecode file]\n")
	break
      end
      local v = fl[n]
      if ann ~= 0 then
	...
	elseif v2 then
	  show = n+ann
	  table.insert(out,format("@@ %d @@\n", n))
	end
	if not show then goto next end
      end
      if v then
	table.insert(out,format(fmtv, v, line))
      else
	table.insert(out,format(fmtn, line))
      end
    ::next::
      n = n + 1
    end
...
local function prof_finish()
  if prof_ud then
    ...
    if samples == 0 then
      if prof_raw ~= true then table.insert(out,"[No samples collected]\n") end
      return
    end
    if prof_ann then
