...
function updateSynthdefGui(parameterCtl,params)
	syntdef=parameterCtl.syntdef
	params= params or {}
	for i,v in ipairs(parameterCtl) do
		if v.label2 then --is hSlider
			v:val(params[v.label])
			guiSetLabel(v.label2.tag,string.format("%4.2f",params[v.label]))
		end
	end
end
