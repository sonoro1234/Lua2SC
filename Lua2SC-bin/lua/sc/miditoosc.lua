...
function EPinstGUI(player,parent)
	...
	function notysink:notify(control)
		...
		local bb ={[control.variable[1]] = FS(function() return actualvalue end,nil,nil,-1)}
		player:MergeBind(bb)
	...
	if midiEvent.type==midi.noteOn then
		...
		for k,v in pairs(thisMidiOsc.params) do
			...
			if type(v)~="table" then
				...
				table.insert(on[2],{"]"})
				--local endpar=#on[2]
				--on.parentesis = on.parentesis or {}
				--table.insert(on.parentesis,{inipar,endpar})
			end
		end
		--print("envio osc\n")
		--prtable(on)
		--assert(false,"ToDO changed TOOSC")
