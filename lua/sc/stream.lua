...
--------------------------------------------------
--custom function as stream provider F(table1,table2) __add and __mul work on table1
...
function FuncStream:pnext(e)
	if self.creps == self.reps then 
		...
		return self.verb(self.argus,self.argusb,e)
	end
end
...
---------------------------------------------------
ListStream = Stream:new{pos=1,argus=nil}
function ListStream:pnext(e)
	if not self.lista then self:reset() end
	while(true) do
		if self.creps == self.reps then -- contrl repeticiones
			self:reset()
			return nil
		else
			local b = self.lista[self.pos]
			if b~=nil then
				self.pos = self.pos + 1
				return b
			else						-- lista ended but still remain repetitions
				self.pos = 1
				self.creps = self.creps + 1
			end
		end
...
function ListStream:merge(t)
	for i,v in ipairs(self.argus) do
		v:merge(t)
	end
end
function ListStream:reset()
	self.lista=self.argus
	assert(#self.lista > 0,"ListStream without list")
	for i,v in ipairs(self.lista) do
...
function LS(t,r)
	...
	return ListStream:new({reps=r,argus=t})
end
function LOOP(t)
	return LS(t,-1)
end
...
function AutoGenListStream:pnext(e)
	if not self.lista then self.lista=self.generator(self.argus) end
	return ListStream.pnext(self,e)
end
function AutoGenListStream:reset()
	self.lista= nil --self.generator(self.argus)
	self.creps=0
	self.pos=1
	self.recur=nil
end
