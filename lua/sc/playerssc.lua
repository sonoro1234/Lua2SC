...
function doOscSchedule(window)
    ...
    repeat        
        ...
        if OsceventQueue[1] and OsceventQueue[1].time<window then
			...
			--udp:send(toOSCSC(OsceventQueue[1]))
...
-----------------------------------------------------
--read lista={name=value,name2=value...} into osc msg
function getMsgLista(msg,lista)
	for name,value in pairs(lista) do
		table.insert(msg[2],name)
		if type(value) ~= "table" then
			table.insert(msg[2],{"float",value})
		else
			table.insert(msg[2],{"["})
